...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const origin = req.headers.get("origin") || "https://www.artistosapp.com"

    // Student discount: 50% off Pro for .edu emails
    const isEdu = userEmail && userEmail.toLowerCase().endsWith(".edu")
    const studentCouponId = Deno.env.get("STRIPE_STUDENT_COUPON_ID") || ""
    ...
    const sessionParams: Record<string, string> = {
      ...
      ...(planId === "pro" && !isEdu ? { "subscription_data[trial_period_days]": "14" } : {}),
    }

    if (isEdu && planId === "pro" && studentCouponId) {
      sessionParams["discounts[0][coupon]"] = studentCouponId
    } else {
