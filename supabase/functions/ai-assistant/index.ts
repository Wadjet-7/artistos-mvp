...
Return ONLY the JSON object with the four fields.`
    ...
    if (action === "describe_artwork") {
      ...
      const artistLoc = String(data.artist?.location || "").toLowerCase()
      const locationOk = (o: any) => {
        const scope = String(o.location_scope || "").toLowerCase()
        if (!["state", "local", "regional"].includes(scope)) return true
        const detail = String(o.location_detail || "").toLowerCase()
        if (!detail || !artistLoc) return true
        const tokens = detail.split(/[,/;·]| and /).map((t: string) => t.trim()).filter((t: string) => t.length > 2)
        return tokens.some((t: string) => artistLoc.includes(t) || t.includes(artistLoc.split(",")[0].trim()))
      }
